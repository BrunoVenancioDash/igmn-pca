...
# ===================== Matriz de Confusao ================================================
def _predizer(igmn, X, n_classes=N_CLASSES):
    """Vetor de predições da IGMN para X (só features)."""
    preds = np.empty(len(X), dtype=int)
    for i, feat in enumerate(X):
        out = np.asarray(igmn.classify(feat)).ravel()
        if out.shape[0] != n_classes:          # robustez à convenção do classify
            out = out[-n_classes:]
        preds[i] = int(np.argmax(out))
    return preds


def analisar_confusao(igmn, X, y, nome='conjunto', n_classes=N_CLASSES, max_n=None):
    """Matriz de confusão + tabela por classe (TP, FP, FN, precisão, recall).

    FP = previstos como a classe c, mas que não são c (falsos positivos).
    FN = são da classe c, mas previstos como outra (falsos negativos).
    'max_n' subamostra (útil no treino, que é grande).
    Retorna (cm, metricas).
    """
    X = np.asarray(X); y = np.asarray(y)
    if max_n and max_n < len(X):
        idx = np.random.RandomState(0).permutation(len(X))[:max_n]
        X, y = X[idx], y[idx]
    pred = _predizer(igmn, X, n_classes)
    cm = confusion_matrix(y, pred, labels=range(n_classes))

    tp  = np.diag(cm)
    fp  = cm.sum(axis=0) - tp      # coluna - diagonal
    fn  = cm.sum(axis=1) - tp      # linha  - diagonal
    sup = cm.sum(axis=1)           # suporte real por classe
    with np.errstate(divide='ignore', invalid='ignore'):
        prec = np.where(tp + fp > 0, tp / (tp + fp), 0.0)
        rec  = np.where(tp + fn > 0, tp / (tp + fn), 0.0)
    acc = tp.sum() / cm.sum()

    print(f"\n=== {nome}: acuracia={acc:.4f}  (n={cm.sum()}) ===")
    print("classe    TP    FP    FN    prec  recall  suporte")
    for c in range(n_classes):
        print(f"   {c:>2}  {tp[c]:>5} {fp[c]:>5} {fn[c]:>5}   "
              f"{prec[c]:.3f}  {rec[c]:.3f}  {sup[c]:>6}")
    print(f" media  {tp.sum():>5} {fp.sum():>5} {fn.sum():>5}   "
          f"{prec.mean():.3f}  {rec.mean():.3f}  {sup.sum():>6}")
    return cm, dict(tp=tp, fp=fp, fn=fn, prec=prec, rec=rec, sup=sup, acc=acc)


def plotar_confusao(cm, nome='conjunto', out_prefix='igmn_confusao', normalizar=True):
    """Exporta o heatmap da matriz de confusao 'cm' (saida de analisar_confusao).
    normalizar=True -> mostra fracao por linha (taxa de acerto/erro por classe);
    normalizar=False -> mostra contagens absolutas.
    """
    cmn = cm.astype(float)
    if normalizar:
        soma = cmn.sum(axis=1, keepdims=True)
        cmn = np.divide(cmn, soma, out=np.zeros_like(cmn), where=soma > 0)

    fig, ax = plt.subplots(figsize=(6.5, 5.5))
    im = ax.imshow(cmn, cmap='Blues', vmin=0, vmax=1 if normalizar else cmn.max())
    n = cm.shape[0]
    ax.set_xticks(range(n)); ax.set_yticks(range(n))
    ax.set_xlabel('classe prevista'); ax.set_ylabel('classe real')
    ax.set_title(f'Matriz de confusão — {nome}')
    thr = (1 if normalizar else cmn.max()) / 2
    for i in range(n):
        for j in range(n):
            val = cmn[i, j]
            if (normalizar and val > 0.005) or (not normalizar and cm[i, j] > 0):
                txt = f"{val:.2f}" if normalizar else f"{int(cm[i, j])}"
                ax.text(j, i, txt, ha='center', va='center', fontsize=7,
                        color='white' if val > thr else 'black')
    fig.colorbar(im, ax=ax, fraction=0.046, pad=0.04)
    fig.tight_layout()
    out = f"{out_prefix}_{nome.lower()}"
    fig.savefig(out + '.png', dpi=200, bbox_inches='tight')
    fig.savefig(out + '.pdf', bbox_inches='tight')
    print(f"[confusão] salvo em {out}.png / .pdf")
    return fig


# uso: aproveita o cm que analisar_confusao ja devolve
cm_te, m_te = analisar_confusao(igmn, Xte_r, yte, nome='TESTE')
plotar_confusao(cm_te, nome='TESTE')

cm_tr, m_tr = analisar_confusao(igmn, Xtr_r, ytr, nome='TREINO')
plotar_confusao(cm_tr, nome='TREINO')